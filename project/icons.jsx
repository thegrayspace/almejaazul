...
// Lightning bolt — WiFi / high-speed connectivity
...
// Beach parasol — seaside / day pass
const IconUmbrella = ({size=24}) => (
  <svg {..._i(size)}>
...
// Twin wave crests — pool / swimming / water access
const IconWaves = ({size=24}) => (
  <svg {..._i(size)}>
...
// Open beach pavilion — cottage / shaded rental
const IconCottage = ({size=24}) => (
  <svg {..._i(size)}>
...
// Dive mask + tube — snorkeling
const IconSnorkel = ({size=24}) => (
  <svg {..._i(size)}>
...
// Speedboat profile — jet ski
const IconJetSki = ({size=24}) => (
  <svg {..._i(size)}>
...
// Elongated hull + riders — banana boat
const IconBananaBoat = ({size=24}) => (
  <svg {..._i(size)}>
...
// Double-bladed paddle — kayaking
const IconKayak = ({size=24}) => (
  <svg {..._i(size)}>
...
// Paddle face + ball — pickleball / paddle sports
const IconPickleball = ({size=24}) => (
  <svg {..._i(size)}>
...
// Signal arcs + dot — high-speed WiFi
...
// Sun high + horizon + upward chevron — sunrise / morning / arrive
const IconSunrise = ({size=24}) => (
  <svg {..._i(size)}>
...
// Coffee cup with steam — breakfast / morning
const IconCoffee = ({size=24}) => (
  <svg {..._i(size)}>
...
// Sun low + horizon — sunset / evening
const IconSunset = ({size=24}) => (
  <svg {..._i(size)}>
...
// Leaf + vein — mangrove / nature trail
...
// Swimmer + waves — infinity pool
const IconPool = ({size=24}) => (
  <svg {..._i(size)}>
...
// Expose all icons globally for use across page scripts
Object.assign(window, {
  IconBolt, IconUmbrella, IconWaves, IconCottage, IconPaw, IconPhone,
  IconSnorkel, IconJetSki, IconBananaBoat, IconKayak, IconPickleball,
  IconDining, IconWifi, IconSunrise, IconCoffee, IconSunset,
  IconLeaf, IconPool
});
