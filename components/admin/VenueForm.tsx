...
const VENUE_TYPES = ['wedding', 'corporate', 'private'];
const FLOOR_PLAN_TYPES = ['MANGROVE_PAVILION', 'VOW_BY_THE_SEA', 'CUSTOM'];

const defaultVenue: VenueData = { type: 'wedding', name: '', tag: '', capacity: '', subtitle: '', description: '', imageUrl: '', modalImageUrl: '', floorPlanSvgType: 'CUSTOM', amenities: [], price: 0, priceMode: 'INQUIRE', customPriceText: '', isPublished: true, sortOrder: 0 };

export default function VenueForm({ venue }: { venue?: VenueData }) {
  const isNew = !venue?.id;
  const [formData, setFormData] = useState<VenueData>(venue ?? defaultVenue);
  const [amenitiesText, setAmenitiesText] = useState((venue?.amenities ?? []).join('\n'));
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  function set<K extends keyof VenueData>(key: K, value: VenueData[K]) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage({ type: 'success', text: isNew ? 'Venue created!' : 'Venue updated!' });
      setTimeout(() => { window.location.href = '/admin/venues'; }, 1500);
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setTimeout(() => { window.location.href = '/admin/venues'; }, 1000);
    } catch (err) {
      setMessage({ type: 'error', text: err instanceof Error ? err.message : 'An error occurred' });
      setLoading(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ background: '#fff', borderRadius: 8, padding: '28px 32px', marginBottom: 16, boxShadow: '0 2px 12px rgba(26,37,48,0.06)' }}>
        <h2 style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(26,37,48,0.4)', marginTop: 0, marginBottom: 24 }}>Venue Info</h2>
        <div style={fieldStyle}><label style={labelStyle}>Venue Name *</label><input style={inputStyle} type="text" value={formData.name} onChange={e => set('name', e.target.value)} required placeholder="e.g. The Mangrove Pavilion" /></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={fieldStyle}><label style={labelStyle}>Type</label><select style={inputStyle} value={formData.type} onChange={e => set('type', e.target.value)}>{VENUE_TYPES.map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}</select></div>
          <div style={fieldStyle}><label style={labelStyle}>Tag</label><input style={inputStyle} type="text" value={formData.tag} onChange={e => set('tag', e.target.value)} placeholder="e.g. Reception" /></div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={fieldStyle}><label style={labelStyle}>Capacity</label><input style={inputStyle} type="text" value={formData.capacity} onChange={e => set('capacity', e.target.value)} placeholder="e.g. Up to 60 guests" /></div>
          <div style={fieldStyle}><label style={labelStyle}>Subtitle</label><input style={inputStyle} type="text" value={formData.subtitle} onChange={e => set('subtitle', e.target.value)} placeholder="e.g. Canopy setting · Intimate" /></div>
        </div>
        <div style={fieldStyle}><label style={labelStyle}>Description *</label><textarea style={{ ...inputStyle, resize: 'vertical' }} rows={4} value={formData.description} onChange={e => set('description', e.target.value)} required placeholder="Full venue description" /></div>
        <div style={fieldStyle}><label style={labelStyle}>Amenities (one per line)</label><textarea style={{ ...inputStyle, resize: 'vertical' }} rows={4} value={amenitiesText} onChange={e => setAmenitiesText(e.target.value)} placeholder="Beach ceremony setup&#10;In-house coordination&#10;Accommodation for wedding party" /></div>
        <div style={fieldStyle}><label style={labelStyle}>Floor Plan Type</label><select style={inputStyle} value={formData.floorPlanSvgType} onChange={e => set('floorPlanSvgType', e.target.value)}>{FLOOR_PLAN_TYPES.map(t => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}</select><p style={{ fontSize: 11, color: 'rgba(26,37,48,0.45)', marginTop: 4 }}>MANGROVE_PAVILION and VOW_BY_THE_SEA use built-in SVG floor plans.</p></div>
      </div>
      <div style={{ background: '#fff', borderRadius: 8, padding: '28px 32px', marginBottom: 16, boxShadow: '0 2px 12px rgba(26,37,48,0.06)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'space-between' }}>
        <button type="submit" disabled={loading} style={{ padding: '12px 32px', background: loading ? 'rgba(75,191,224,0.5)' : '#4BBFE0', color: '#1a2530', border: 'none', borderRadius: 100, fontFamily: 'inherit', fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer' }}>{loading ? 'Saving…' : isNew ? 'Create Venue' : 'Save Changes'}</button>
        {!isNew && <button type="button" onClick={handleDelete} disabled={loading} style={{ padding: '12px 24px', background: 'transparent', color: '#c0392b', border: '1.5px solid rgba(192,57,43,0.3)', borderRadius: 100, fontFamily: 'inherit', fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer' }}>Delete Venue</button>}
      </div>
    </form>
  );
}
