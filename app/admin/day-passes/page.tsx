  ...
  return (
    <AdminShell title="Day Passes">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        ...
        <Link href="/admin/day-passes/new" style={{ padding: '10px 24px', background: '#4BBFE0', color: '#1a2530', textDecoration: 'none', borderRadius: 100, fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' }}>+ Add Pass</Link>
      </div>
      {passes.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {passes.map(pass => (
            <div key={pass.id} style={{ background: '#fff', borderRadius: 8, padding: '18px 24px', boxShadow: '0 2px 12px rgba(26,37,48,0.06)', display: 'flex', alignItems: 'center', gap: 20 }}>
              ...
              <Link href={`/admin/day-passes/${pass.id}`} style={{ padding: '8px 20px', background: 'transparent', color: '#1a2530', textDecoration: 'none', borderRadius: 100, fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', border: '1.5px solid rgba(26,37,48,0.2)', flexShrink: 0 }}>Edit</Link>
            </div>
          ))}
        </div>
      )}
