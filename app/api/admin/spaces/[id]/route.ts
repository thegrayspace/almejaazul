...
const SpaceUpdateSchema = z.object({
  ...
  priceSub: z.string().max(200).optional(),
...
export async function GET(_req: NextRequest, { params }: RouteContext) {
  ...
  return NextResponse.json({ ...space, price: space.price.toNumber() });
}
...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  const existing = await prisma.bookableSpace.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  const space = await prisma.bookableSpace.update({ where: { id }, data: parsed.data });
  return NextResponse.json({ ...space, price: space.price.toNumber() });
}
