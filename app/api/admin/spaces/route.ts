...
const SpaceCreateSchema = z.object({
  ...
  priceSub: z.string().max(200).optional().default(''),
...
export async function GET() {
  ...
  return NextResponse.json(spaces.map(s => ({ ...s, price: s.price.toNumber() })));
}
...
export async function POST(req: NextRequest) {
  ...
  const space = await prisma.bookableSpace.create({ data: parsed.data });
  return NextResponse.json({ ...space, price: space.price.toNumber() }, { status: 201 });
}
